...
func (r *Reporter) FlushOnce() error {
	r.Registry.Each(func(metricName string, i interface{}) {
		...
		case metrics.Counter:
			v := metric.Count()
			l := r.ss[name]
			// do nothing, metrics are best effort
			_ = r.Client.Count(name, v-l, tags, 1)

			r.ss[name] = v

		case metrics.Gauge:
			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name, float64(metric.Value()), tags, 1)

		case metrics.GaugeFloat64:
			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name, metric.Value(), tags, 1)

		case metrics.Histogram:
			ms := metric.Snapshot()

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".count", float64(ms.Count()), tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".max", float64(ms.Max()), tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".min", float64(ms.Min()), tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".mean", ms.Mean(), tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".stddev", ms.StdDev(), tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".sum", float64(ms.Sum()), tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".var", ms.Variance(), tags, 1)

			if len(r.percentiles) > 0 {
				values := ms.Percentiles(r.percentiles)
				for i, p := range r.p {
					// do nothing, metrics are best effort
					_ = r.Client.Gauge(name+p, values[i], tags, 1)
		...
		case metrics.Meter:
			...
			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".rate1", ms.Rate1(), tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".rate5", ms.Rate5(), tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".rate15", ms.Rate15(), tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".mean", ms.RateMean(), tags, 1)
		...
		case metrics.Timer:
			...
			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".max", time.Duration(ms.Max()).Seconds()*1000, tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".min", time.Duration(ms.Min()).Seconds()*1000, tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".mean", time.Duration(ms.Mean()).Seconds()*1000, tags, 1)

			// do nothing, metrics are best effort
			_ = r.Client.Gauge(name+".stddev", time.Duration(ms.StdDev()).Seconds()*1000, tags, 1)
			...
			if len(r.percentiles) > 0 {
				...
				for i, p := range r.p {
					// do nothing, metrics are best effort
					_ = r.Client.Gauge(name+p, time.Duration(values[i]).Seconds()*1000, tags, 1)
